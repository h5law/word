...
uint32_t widget_uc_sanitize(uint32_t uc, int *width);
int      widget_str_width(const char *str);
bool     widget_points_in_bounds(const struct widget_points *points, int x,
                                 int y);
void widget_points_set(struct widget_points *points, int x1, int x2, int y1,
                       int y2);
bool widget_should_forcebreak(int width);
...
bool widget_should_scroll(int x, int width, int max_width);
...
bool widget_advance_xy_if_scroll(int *x, int *y, struct widget_points *points,
                                 int width);
int  widget_print_str(int x, int y, int max_x, uintattr_t fg, uintattr_t bg,
                      const char *str);
int  widget_pad_center(int part, int total);

/* Border */
void border_redraw(struct widget_points *points, uintattr_t fg,
                   uintattr_t bg);
...
int  input_init(struct input *input, uintattr_t bg, bool scroll_horizontal);
void input_finish(struct input *input);
/* rows will be filled with the number of rows taken by the input field. */
void input_redraw(struct input *input, struct widget_points *points,
                  int *rows, bool dry_run);
enum widget_error input_handle_event(struct input    *input,
                                     enum input_event event, ...);
char             *input_buf(struct input *input);

/* Treeview. */

/* Called to draw the data. */
typedef void (*treeview_draw_cb)(void *data, struct widget_points *points,
                                 bool is_selected);
...
enum treeview_event {
    ...
    /* Jump to a node, pass a pointer to a treeview struct
     * parent->nodes[index]
     */
    TREEVIEW_JUMP,
...
struct treeview_node *treeview_node_alloc(void            *data,
                                          treeview_draw_cb draw_cb);
int  treeview_node_init(struct treeview_node *node, void *data,
                        treeview_draw_cb draw_cb);
void treeview_node_destroy(struct treeview_node *node);
void treeview_node_finish(struct treeview_node *node);
int  treeview_node_add_child(struct treeview_node *parent,
                             struct treeview_node *child);
int  treeview_init(struct treeview *treeview);
void treeview_finish(struct treeview *treeview);
void treeview_redraw(struct treeview *treeview, struct widget_points *points);
enum widget_error treeview_event(struct treeview    *treeview,
                                 enum treeview_event event, ...);
...
bool widget_points_in_bounds(const struct widget_points *points, int x, int y)
{
    return (x >= points->x1 && x < points->x2 && y >= points->y1 &&
            y < points->y2);
}
...
bool widget_should_forcebreak(int width) { return width == 0; }
...
void border_redraw(struct widget_points *points, uintattr_t fg, uintattr_t bg)
{
    if (!points) {
        return;
    }
    ...
    for (int x = points->x1 + 1; x < (points->x2 - 1); x++) {
        widget_print_str(x, points->y1, points->x2, fg, bg,
                         borders[BORDER_NORMAL]);
    }

    widget_print_str(points->x1, points->y1, points->x2, fg, bg,
    ...
    for (int y = points->y1 + 1; y < points->y2; y++) {
        widget_print_str(points->x1, y, points->x2, fg, bg,
                         borders[BORDER_VERTICAL]);
        widget_print_str(points->x2 - 1, y, points->x2, fg, bg,
                         borders[BORDER_VERTICAL]);
    }
    ...
    if (height > 2) {
        ...
        for (int x = points->x1 + 1; x < (points->x2 - 1); x++) {
            widget_print_str(x, points->y2 - 1, points->x2, fg, bg,
                             borders[BORDER_NORMAL]);
        }
    }
}
...
static enum widget_error buf_add(struct input *input, uint32_t ch)
{
...
static enum widget_error buf_left(struct input *input)
{
    if (input->cur_buf > 0) {
        input->cur_buf--;
...
static enum widget_error buf_leftword(struct input *input)
{
    if (input->cur_buf > 0) {
        do {
            input->cur_buf--;
...
static enum widget_error buf_right(struct input *input)
{
...
static enum widget_error buf_rightword(struct input *input)
{
...
static enum widget_error buf_del(struct input *input)
{
    if (input->cur_buf > 0) {
        --input->cur_buf;
...
static enum widget_error buf_delword(struct input *input)
{
    ...
    *rows = 0;
...

    {
        ...
        for (size_t written = 0; written < buf_len; written++) {
            ...
            /* Check if adding another character would overflow the screen.
             * This prevents us from having the cursor stuck in the 1px gap
             * between points->x2 and points->x2 - 1 if the character was an
             * emoji. */
...
static bool is_last(const struct treeview_node *node)
{
    size_t len = node && node->parent ? arrlenu(node->parent->nodes) : 0;
    return (len > 0 && node == node->parent->nodes[len - 1]);
}

static struct treeview_node *leaf(struct treeview_node *node)
{
...
static struct treeview_node *parent_next(struct treeview_node *node)
{
    if (node->parent) {
...
static int node_height_up_to_bottom(struct treeview_node *node)
{
    assert(node);
...
static int node_height_bottom_to_up(struct treeview_node *node)
{
    assert(node);
...
static int redraw(struct treeview *treeview, const struct treeview_node *node,
                  const struct widget_points *points, int x, int y)
{
    ...
    if (node->parent && treeview->skipped++ >= treeview->start_y) {
        ...
        widget_points_set(&user_points, x + symbol_printed_width, points->x2,
                          y, points->y2);
...
static void node_children_destroy(struct treeview_node *node)
{
    if (node->nodes) {
        for (size_t i = 0, len = arrlenu(node->nodes); i < len; i++) {
                                 ...
                                 enum treeview_event event, ...)
{
    ...
    case TREEVIEW_UP:
        ...
        if (treeview->selected->parent->index > 0) {
            ...
            treeview->start_y = 0; /* Scroll up to the title if we're already
    ...
    case TREEVIEW_DOWN:
        ...
        if (treeview->selected->is_expanded &&
            ...
            treeview->selected =
                    treeview->selected->nodes[0]; /* First node. */
        } else {
...
#undef print_str
...

    {
        ...
        for (size_t i = 0; i < BUF_MAX; i++) {
            assert(input_handle_event(&input, INPUT_ADD, ' ') ==
                   WIDGET_REDRAW);
        }
        ...
        assert(input_handle_event(&input, INPUT_DELETE_WORD) ==
               WIDGET_REDRAW);
        assert(input_buf(&input) == NULL);
